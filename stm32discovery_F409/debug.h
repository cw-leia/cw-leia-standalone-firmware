...
#if DEBUG_LVL >= 3
#define LOG(fmt, ...)                                                          \
  dbg_log("%lld: [II] %s:%d, %s:" fmt, get_ticks(), __FILE__, __LINE__,        \
...
#if DEBUG_LVL >= 2
#define WARN(fmt, ...)                                                         \
  dbg_log("%lld: [WW] %s:%d, %s:" fmt, get_ticks(), __FILE__, __LINE__,        \
          __FUNCTION__, ##__VA_ARGS__)
#else
#define WARN(fmt, ...)                                                         \
  do {                                                                         \
...
#define ERROR(fmt, ...)                                                        \
  do {                                                                         \
    dbg_log("%lld: [EE] %s:%d, %s:" fmt, get_ticks(), __FILE__, __LINE__,      \
            __FUNCTION__, ##__VA_ARGS__);                                      \
    /*if (logging)*/                                                           \
    dbg_flush();                                                               \
  } while (0)
