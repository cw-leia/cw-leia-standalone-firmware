...
static int SC_negotiate_PTS(SC_ATR *atr, uint8_t *T_protocol, uint8_t do_negotiate_pts, uint8_t do_change_baud_rate, uint8_t do_force_protocol, uint32_t do_force_etu, uint32_t do_force_freq){
	...
	*T_protocol = 0;
	...
	if(do_change_baud_rate){
		...
		/* Is the user forcing a freq ? */
		if(do_force_freq){
			log_printf("[Smartcard] Current maximum f_max_curr=%d, user asked for %d ...\n", atr->f_max_curr, do_force_freq);
			if(platform_smartcard_clock_rounding(do_force_freq, &(atr->f_max_curr)) != 0){
				goto err;
			}

			log_printf("[Smartcard] Trying f_curr=%d as the best choice compared to asked %d\n", atr->f_max_curr, do_force_freq);
		}
		...
		if(atr->t_mask[2] & 0x1){
			...
			if(extra_guard_time == 0xff){
				if(*T_protocol == 0){
					 ...
					 * might not handle such a parameter, hence we ask for this in the underlying layer or
					 * return an error.
					 */
...
static int SC_send_APDU_T0(SC_APDU_cmd *apdu, SC_APDU_resp *resp){
	...
	if(apdu->lc > SHORT_APDU_LC_MAX){
		...
		trig(TRIG_SEND_APDU_FRAGMENTED_T0_PRE);
		...
		curr_resp.sw1 = curr_resp.sw2 = curr_resp.le = 0;
		trig(TRIG_SEND_APDU_SIMPLE_T0_PRE);
		if(SC_push_pull_APDU_T0(&curr_apdu, &curr_resp)){
			goto err;
		}
	}
	...
	if(((curr_resp.sw1 == 0x61) && (apdu->send_le != 0) && (apdu->le > SHORT_APDU_LE_MAX)) || (case4_getresponse == 1)){
		...
		case4_getresponse = 0;
		trig(TRIG_GET_RESP_FRAGMENTED_T0_PRE);
		while(1){
		...
		trig(TRIG_GET_RESP_SIMPLE_T0_PRE);
		resp->le = curr_resp.le;
		resp->sw1 = curr_resp.sw1;
		resp->sw2 = curr_resp.sw2;
		memcpy(resp->data, curr_resp.data, curr_resp.le);
...
int SC_iso7816_fsm_init(SC_ATR *atr, uint8_t *T_protocol, uint8_t do_negiotiate_pts, uint8_t do_change_baud_rate, uint8_t do_force_protocol, uint32_t do_force_etu, uint32_t do_force_freq){
	int ret;
