...
int SC_fsm_init(SC_Card *card, uint8_t do_negiotiate_pts, uint8_t do_change_baud_rate, uint8_t do_force_protocol, uint32_t do_force_etu, uint32_t do_force_freq){
        if(card == NULL){
                return -1;
        }

	/* Try to init in all the modes we have, contact and then contactless  */
	if(SC_iso7816_fsm_init(&(card->info.atr), &(card->T_protocol), do_negiotiate_pts, do_change_baud_rate, do_force_protocol, do_force_etu, do_force_freq) == 0){
