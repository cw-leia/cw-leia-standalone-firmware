 ...
 * Working buffers of 16384 should do the trick.
 */
#if (CONSOLE_TX_RINGBUFF_SIZE < 11)
#error "APDU size error!!"
#endif
#define APDU_MAX_BUFF_LEN  (CONSOLE_TX_RINGBUFF_SIZE - 32)
...
typedef struct __attribute__((packed)) {
    ...
    uint32_t delta_t; /* Total time of the APDU in microseconds */
    uint32_t delta_t_answer; /* Answer time of the APDU in microseconds */
...
int SC_fsm_init(SC_Card* card, uint8_t do_negotiate_pts, uint8_t do_change_baudrate, uint8_t do_force_protocol, uint32_t do_force_etu, uint32_t do_force_freq, uint8_t wait_for_card);
