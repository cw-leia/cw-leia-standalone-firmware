...
/* Upper layer wait time extension handling */
extern void __protocol_send_wait_extension(uint8_t);
...
typedef enum {
    ...
    SC_TS_UNKOWN_CONVENTION = 2,
} Sc_convention;
static volatile uint8_t SC_convention = SC_TS_UNKOWN_CONVENTION;

static uint8_t SC_is_inverse_conv(void)
{
    if(SC_convention != SC_TS_INVERSE_CONVENTION) {
        return 0;
...
static int SC_set_unknown_conv(void)
{
    /* Set unkown convention */
    SC_convention = SC_TS_UNKOWN_CONVENTION;

    return platform_SC_set_unkown_conv();
}
...
static int SC_getc_timeout(uint8_t* c, uint32_t etu_timeout)
{
    ...
    while(platform_SC_getc(c, t, 0)) {
        if(etu_timeout != 0) {
            ...
            if((curr_tick - start_tick) > (uint32_t)t) {
...
int SC_get_ATR(SC_ATR* atr)
{
    ...
    log_printf_delay("SC_get_ATR\r\n");

    /* Default values per standard */
    atr->D_i_curr = 1;
    atr->F_i_curr = SMARTCARD_DEFAULT_ETU;
    atr->f_max_curr = SC_get_sc_clock_freq();
     ...
     */
    /******************************/
    trig(TRIG_GET_ATR_PRE);

    if(SC_set_unknown_conv()){
        goto err;
    }
    ...
    if(do_check_checksum) {
        ...
        /* Check the checksum */
        if(checksum != atr->tck) {
            log_printf_delay("Smartcard ATR checksum error ...\r\n");
            goto err;
        }
    }
 ...
 */
static int SC_negotiate_PTS(SC_ATR* atr, uint8_t* T_protocol, uint8_t do_negotiate_pts, uint8_t do_change_baud_rate, uint8_t do_force_protocol, uint32_t do_force_etu, uint32_t do_force_freq)
{
	uint8_t ta1, td1;
	uint32_t etu_curr = 0;
	uint8_t negotiable_mode = 1;
	uint8_t extra_guard_time = 1;
	uint8_t asked_ta1 = 0, __attribute__((unused)) asked_tc1 = 0;
	uint8_t pck;
	uint8_t c;
	...
	if(atr->t_mask[0] & (0x1 << 1)){
		...
		log_printf_delay("[Smartcard] TA2 present, implies specific mode\r\n");
		negotiable_mode = 0;
	}
	else{
		/* The card supports negotiable mode */
		log_printf_delay("[Smartcard] TA2 absent, implies negotiable mode\r\n");
		negotiable_mode = 1;
	}
	...
	*T_protocol = 0;
	...
	if((atr->t_mask[3] & 0x1) && (do_force_protocol == 0)){
		td1 = atr->td[0];
		*T_protocol = td1 & 0x0f;
		log_printf_delay("[Smartcard] ATR prefered protocol T=%d\r\n", *T_protocol);
		if((*T_protocol != 0) && (*T_protocol != 1)){
			/* We do not support T=15 or protocols other than T=0/T=1 */
			log_printf_delay("[Smartcard] Asking for unsupported protocol T=%d\r\n", *T_protocol);
			goto err;
		}
	}
	if(do_force_protocol){
		if(do_force_protocol > 2){
			log_printf_delay("[Smartcard] Asking for unsupported *user forced* protocol T=%d\r\n", (do_force_protocol - 1));
			goto err;
		}
	...
	if(do_change_baud_rate){
		...
		if(do_force_etu){
			if(atr->D_i_curr != 0){
				log_printf_delay("[Smartcard] Current maximum Fi=%d, Di=%d, ETU=%d, user asked for %d ...\r\n", atr->F_i_curr, atr->D_i_curr, atr->F_i_curr / atr->D_i_curr, do_force_etu);
			}
			else{
				log_printf_delay("[Smartcard] Current maximum Fi=%d, Di=%d, user asked for %d ...\r\n", atr->F_i_curr, atr->D_i_curr, do_force_etu);
			...
			while(1){
				if(i == 0){
					log_printf_delay("[Smartcard] Error: forcing ETU to %d failed ...\r\n", do_force_etu);
					goto err;
				}
				if(D_i[i] != 0){
					if((D_i[i] & 0xffff) == 0){
		...
		if(negotiable_mode == 0){
                        if(do_force_freq != 0){
				log_printf_delay("[Smartcard] User asked for frequency %d\r\n", do_force_freq);
                        	/* We are asked to modify the frequency */
                        	atr->f_max_curr = do_force_freq;
                        }
			if(SC_adapt_clocks(etu_curr, atr->f_max_curr)){
				goto err;
			}
                        atr->f_max_curr = SC_get_sc_clock_freq();
			log_printf_delay("[Smartcard] Switching to ETU = %d (Di = %d, Fi = %d), guard time = %d, max_frequency = %d, Protocol T=%d\r\n", etu_curr, atr->D_i_curr, atr->F_i_curr, extra_guard_time, atr->f_max_curr, *T_protocol);
		}
	}
			...
			if(*T_protocol == 0){
				 ...
				 */
                                if(platform_smartcard_set_1ETU_guardtime()){
                                        log_printf_delay("[Smartcard] PSS/PTS error: TC1=0xff for T=1 means unsupported ETU=1 ...\r\n");
                                        goto err;
                                }
				extra_guard_time = 0;
			}
	...
	if(do_negotiate_pts){
		...
		if(asked_ta1){
			if(do_change_baud_rate){
				...
				if(SC_putc_timeout(new_baud_rate, WT_wait_time)){
					log_printf_delay("[Smartcard] PSS/PTS error (byte PTS1)\r\n");
					goto err;
				}
				pck ^= new_baud_rate;
			}
		...
		if(SC_putc_timeout(pck, WT_wait_time)){
			log_printf_delay("[Smartcard] PSS/PTS error (byte PTS checksum)\r\n");
			goto err;
		}
		/* Now check that the card agrees to our PTS */
		/* Check for PSS = 0xff */
		if(SC_getc_timeout(&c, WT_wait_time)){
			log_printf_delay("[Smartcard] PSS/PTS error (0xff ACK error in receive)\r\n");
			goto err;
		}
		if(c != 0xff){
			log_printf_delay("[Smartcard] PSS/PTS error (0xff ACK error, values differ)\r\n");
			goto err;
		}
		/* Check for PTS0 */
		if(SC_getc_timeout(&c, WT_wait_time)){
			log_printf_delay("[Smartcard] PSS/PTS error (PTS0 ACK error in receive)\r\n");
			goto err;
		}
		if(c != ((asked_ta1 << 4) | (*T_protocol))){
			log_printf_delay("[Smartcard] PSS/PTS error (PTS0 ACK error, values differ)\r\n");
			goto err;
		}
		/* Optionally check for PTS1 */
		if(asked_ta1){
			if(SC_getc_timeout(&c, WT_wait_time)){
				log_printf_delay("[Smartcard] PSS/PTS error (PTS1 ACK error in receive)\r\n");
				goto err;
			}
			if(do_change_baud_rate){
				if(c != new_baud_rate){
					log_printf_delay("[Smartcard] PSS/PTS error (PTS1 ACK error, values differ)\r\n");
					goto err;
				}
			}
			else{
		...
		if(SC_getc_timeout(&c, WT_wait_time)){
			log_printf_delay("[Smartcard] PSS/PTS error (PTS checksum ACK error in receive)\r\n");
			goto err;
		}
		if(c != pck){
			log_printf_delay("[Smartcard] PSS/PTS error (PTS checksum ACK error, values differ)\r\n");
			goto err;
		}
		log_printf_delay("[Smartcard] PSS/PTS success: PTS sent and confirmed by the card!\r\n");
	}
...
WAIT_AGAIN:
	...
	if(resp->data[resp->le] == 0x60){
		 ...
		 * telling us to wait ...
		 */
                /* Tell upper layer to extend waiting */
                __protocol_send_wait_extension(1);
		goto WAIT_AGAIN;
	}
	...
	if(apdu->lc > SHORT_APDU_LC_MAX){
		...
		if(((encapsulated_apdu_len % SHORT_APDU_LC_MAX) == 0) && (encapsulated_apdu_len != 0)){
			num_t0_apdus--;
                ...
                trig(TRIG_PRE_SEND_APDU_FRAGMENTED_T0);

		/* Send fragmented T=0 APDUs */
		for(i = 0; i < num_t0_apdus; i++){
			int ret;
	                /* Tell upper layer to extend waiting */
        	        __protocol_send_wait_extension(0);
			...
			if(((curr_resp.sw1 != 0x90) && (curr_resp.sw2 != 0x00)) && (i != (num_t0_apdus-1))){
				...
				local_memcpy(resp->data, curr_resp.data, curr_resp.le);
				goto end;
			}
		}
		/* From here, we continue to getting the answer from the card */
        	resp->delta_t_answer = platform_get_microseconds_ticks();
	}
	else{
		...
		local_memcpy(curr_apdu.data, apdu->data, apdu->lc);
		curr_resp.sw1 = curr_resp.sw2 = curr_resp.le = 0;
        	
        	resp->delta_t_answer = platform_get_microseconds_ticks();
                trig(TRIG_PRE_SEND_APDU_SHORT_T0);
	...
	if(((curr_resp.sw1 == 0x61) && (apdu->send_le != 0) && ((apdu->le > SHORT_APDU_LE_MAX) || (apdu->le == 0x00))) || (case4_getresponse == 1) || (case2_getresponse == 1)){
		...
		while(1){
			...
			if((curr_resp.sw1 == 0x61) || ((curr_resp.sw1 == 0x90) && (curr_resp.sw2 == 0x00)) || (trim_response == 1)){
				...
				local_memcpy(&(resp->data[resp->le]), curr_resp.data, curr_resp.le);
				resp->le += curr_resp.le;
				...
				local_memcpy(resp->data, curr_resp.data, resp->le);
				goto end;
			}
			else{
				/* Sanity check */
...
static int SC_send_APDU_T1(SC_APDU_cmd* apdu, SC_APDU_resp* resp, SC_ATR* atr)
{
	...
	/* Sanity zeroize */
	local_memset(&tpdu_rcv, 0, sizeof(tpdu_rcv));
	local_memset(&tpdu_send, 0, sizeof(tpdu_send));
	/* NAD is always zero in our case (no multi-slaves) */
	tpdu_send.nad = 0;
	/* Send all the IBLOCKS */
        trig(TRIG_PRE_SEND_APDU_T1);
					...
					if(SC_TPDU_T1_is_SBLOCK(&tpdu_rcv)){
						...
						if(SC_TPDU_T1_SBLOCK_get_type(&tpdu_rcv) == SBLOCK_CHANGE_IFS_REQ){
							...
							if((new_ifsc == 0) || (new_ifsc == 255)){
								...
								/* Record the new current IFSC in our ATR context: this will be active
...
void SC_print_TPDU(SC_TPDU* tpdu)
{
    ...
    log_printf("===== TPDU ============\r\n");
    log_printf("NAD = %02x, PCB = %02x, LEN = %02x\r\n", tpdu->nad, tpdu->pcb, tpdu->len);
    ...
    if(tpdu->edc_type == EDC_TYPE_LRC) {
        log_printf("EDC (LRC) = %02x\r\n", tpdu->edc_lrc);

    } else {
        log_printf("EDC (CRC) = %02x\r\n", tpdu->edc_crc);
    }
...
static int SC_reinit_iso7816(uint32_t do_force_freq)
{
    ...
    if(do_force_freq != 0){
        log_printf_delay("[Smartcard] Reinit: User asked for frequency %d\r\n", do_force_freq);
	if(SC_adapt_clocks(SMARTCARD_DEFAULT_ETU, do_force_freq)){
		goto err;
	}
	log_printf_delay("[Smartcard] Reinit: Switching to max_frequency = %d\r\n", SC_get_sc_clock_freq());
    }
    else{
...
int SC_iso7816_fsm_init(SC_ATR* atr, uint8_t* T_protocol, uint8_t do_negiotiate_pts, uint8_t do_change_baud_rate, uint8_t do_force_protocol, uint32_t do_force_etu, uint32_t do_force_freq, uint8_t wait_for_card)
{
    int ret;
...
//SC_READER_IDLE_LABEL:
        ...
        log_printf("/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\\r\n");
        log_printf("Looking for card insertion\r\n");

        if(!platform_is_smartcard_inserted()) {
            log_printf("Card not found...\r\n");
        ...
        if(SC_reinit_iso7816(do_force_freq)) {
            return -1;
        }
...
SC_PROTOCOL_NEG_LABEL:
        ...
        /* Negotiate PTS */
        ret = SC_negotiate_PTS(atr, T_protocol, do_negiotiate_pts, do_change_baud_rate, do_force_protocol, do_force_etu, do_force_freq);
    ...
    default:
        log_printf("Smartcard unhandled state %d in initialization FSM\r\n", SC_current_state);
        return -1;
    }
