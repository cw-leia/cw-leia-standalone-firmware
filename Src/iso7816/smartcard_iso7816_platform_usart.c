...
static LL_GPIO_InitTypeDef gpio_smartcard_contact = {
...
static volatile uint8_t usart_platform_SC_gpio_smartcard_contact_changed = 0;
static volatile uint8_t usart_platform_SC_is_smartcard_inserted = 0;

uint8_t usart_platform_SC_is_lost(void) {
    return ((~usart_platform_SC_is_smartcard_inserted) & 0x1);
}
...
void usart_platform_smartcard_contact_handler(void)
{
    if (LL_EXTI_IsActiveFlag_0_31(SMARTCARD_CONTACT_EXTI_LINE) != RESET) {
        ...
        usart_platform_SC_is_smartcard_inserted = LL_GPIO_IsInputPinSet(SMARTCARD_CONTACT_PORT, SMARTCARD_CONTACT_PIN)  & 1;
#else
        usart_platform_SC_is_smartcard_inserted = (~LL_GPIO_IsInputPinSet(SMARTCARD_CONTACT_PORT, SMARTCARD_CONTACT_PIN))  & 1;
#endif
#else
        usart_platform_SC_is_smartcard_inserted = 1;
#endif
        /* Handle our LED */
        /* Activate LED or not depending on smartcard presence */
        if(usart_platform_SC_is_smartcard_inserted) {
            led_status_on();
        } else {
            led_status_off();
        }
    ...
    usart_platform_SC_gpio_smartcard_contact_changed = 1;
...
{
    usart_platform_SC_gpio_smartcard_contact_changed = 0;
...
{
    //usart_platform_SC_is_smartcard_inserted = 0;
    usart_platform_SC_gpio_smartcard_contact_changed = 1;
...
uint8_t usart_platform_is_smartcard_inserted(void)
{
    ...
    return usart_platform_SC_is_smartcard_inserted;
#else
    return 1;
#endif
}
...
static inline void usart_platform_io_dir_read(void)
{
...
static inline void usart_platform_io_dir_write(void)
{
#ifdef SMARTCARD_IO_DIR_PORT
...
void usart_platform_init_io_dir(void)
{
#ifdef SMARTCARD_IO_DIR_PORT
    LL_GPIO_Init(SMARTCARD_IO_DIR_PORT, &gpio_smartcard_io_dir);
    usart_platform_io_dir_read();
#endif
}
...
void usart_platform_init_smartcard_rst(void)
{
...
void usart_platform_init_smartcard_vcc(void)
{
...
void usart_platform_init_smartcard_vpp(void)
{
...
void usart_platform_set_smartcard_vpp(uint8_t val)
{
#ifdef LEIA
    // Invert
    val = (~val) & 0x1;
...
void
usart_platform_init_smartcard_ioclk(void)
{
...
static int usart_platform_get_bus_clock(USART_TypeDef* USARTx, uint32_t* bus_clock);
...
int usart_platform_smartcard_clock_rounding(uint32_t target_freq, uint32_t* target_freq_rounded)
{
    ...
    /* Get the usart clock */
    if (usart_platform_get_bus_clock(SMARTCARD_USART, &usart_bus_clk) != 0) {
        goto err;
    }
...
err:
    log_printf("Error in usart_platform_get_bus_clock\r\n");
    return -1;
}

static volatile uint8_t usart_platform_SC_pending_receive_byte = 0;
static volatile uint8_t usart_platform_SC_pending_send_byte = 0;
static volatile uint8_t usart_platform_SC_byte = 0;
...
static inline int usart_platform_get_usart_number(USART_TypeDef* USARTx)
{
#if defined(USART1)
    ...
    log_printf("Error in usart_platform_get_usart_number\r\n");
...
static int usart_platform_smartcard_hw_init(smartcard_init_type type)
{
    log_printf("[Low level SC] USART mode init\r\n");

    if((type == GPIOS_EN) || (type == ALL_EN)) {
        /* Initialize the GPIOs */
        usart_platform_init_smartcard_contact();
        usart_platform_init_smartcard_vcc();
        usart_platform_init_smartcard_vpp();
        usart_platform_init_smartcard_rst();
        usart_platform_init_smartcard_ioclk();
        usart_platform_init_io_dir();
    }

    if((type == USART_EN) || (type == ALL_EN)) {
        /* Initialize the USART in smartcard mode */
        LL_USART_Disable(SMARTCARD_USART);
        log_printf("\t==> Init USART%d in smartcard mode!\r\n", usart_platform_get_usart_number(SMARTCARD_USART));
...
int usart_platform_smartcard_init(void)
{
    ...
    usart_platform_SC_pending_receive_byte = 0;
    usart_platform_SC_pending_send_byte = 0;
    usart_platform_SC_byte = 0;
     ...
     * buffers.
     */
    usart_platform_smartcard_hw_init(USART_DIS);

    return usart_platform_smartcard_hw_init(ALL_EN);
}
...
int usart_platform_SC_adapt_clocks(uint32_t* etu, uint32_t* frequency)
{
    ...
    if(usart_platform_smartcard_clock_rounding(*frequency, frequency) != 0) {
        goto err;
    }
...
static inline uint8_t usart_platform_SC_char_pop(void);
static inline void usart_platform_SC_char_push(uint8_t c);
...
{
    if(LL_USART_IsActiveFlag_PE(SMARTCARD_USART) && (usart_platform_SC_pending_send_byte != 0)) {
        /* Parity error, program a resend */
        usart_platform_SC_pending_send_byte = 3;
    ...
    if(LL_USART_IsActiveFlag_FE(SMARTCARD_USART) && (usart_platform_SC_pending_send_byte != 0)) {
        /* Frame error, program a resend */
        usart_platform_SC_pending_send_byte = 4;
        /* Dummy read of the DR register to ACK the interrupt */
    ...
    if(LL_USART_IsActiveFlag_TC(SMARTCARD_USART) && (usart_platform_SC_pending_send_byte != 0)) {
        /* Signal that the byte has been sent */
        usart_platform_SC_pending_send_byte = 2;
        /* Clear TC */
        LL_USART_ClearFlag_TC(SMARTCARD_USART);
    ...
    if(LL_USART_IsActiveFlag_RXNE(SMARTCARD_USART)) {
        usart_platform_SC_byte = usart_platform_SC_char_pop();

        if(usart_platform_SC_pending_send_byte == 0) {
            trig(TRIG_IRQ_GETC);
            usart_platform_SC_pending_receive_byte = 1;
        }
...
void usart_platform_SC_flush(void)
{
     ...
     * our ring buffer!
     */
    usart_platform_SC_pending_receive_byte = usart_platform_SC_pending_send_byte = 0;
}
...
int usart_platform_SC_getc(uint8_t* c, __attribute__((unused)) uint32_t timeout, __attribute__((unused)) uint8_t reset)
{
    ...
    if(usart_platform_SC_pending_receive_byte != 1) {
        return -1;
    }

    /* Data is ready, go ahead */
    *c = usart_platform_SC_byte;
    usart_platform_SC_pending_receive_byte = 0;
 ...
 */
int usart_platform_SC_putc(uint8_t c, __attribute__((unused)) uint32_t timeout, __attribute__((unused)) uint8_t reset)
{
    ...
    if((usart_platform_SC_pending_send_byte == 0) || (usart_platform_SC_pending_send_byte >= 3)) {
        usart_platform_io_dir_write();
        usart_platform_SC_pending_send_byte = 1;
        usart_platform_SC_char_push(c);
        return -1;
    }

    if(usart_platform_SC_pending_send_byte == 2) {
        usart_platform_io_dir_read();
...
{
    log_printf("[Low level SC] USART mode deinit\r\n");

    setup_microsec_timer_done = 0;
    /* Mainly stop our timers and disable the ISO7816 UART */
    LL_TIM_DeInit(TIM5);
